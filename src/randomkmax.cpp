...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	...
	if ((nrhs <= 2) && (nrhs > 3)){
		mexErrMsgIdAndTxt("MATLAB:randomkmax:invalidNumInputs", "2 or 3 inputs required the array and k.");
	}
	if (!mxIsScalar(prhs[1])) {
		mexErrMsgIdAndTxt("MATLAB:randomkmax:inputNotScalar",
...
/* Create matrix for the return argument. */
	...
	switch(dataType) {
		case mxSINGLE_CLASS:
			fKst::findKBigest((float*) ptrData, sizeArray, k, (float*) ptrResult, (unsigned int*)ptrPosition, rng);
			break;
		case mxDOUBLE_CLASS:
			fKst::findKBigest((double*) ptrData, sizeArray, k, (double*) ptrResult, (unsigned int*)ptrPosition, rng);
			break;

		// you can have any number of case statements.
		default : //Optional
		mexErrMsgIdAndTxt("MATLAB:randomkmax:invalidNumInputs", "dataType not supported");
	}
